...
def collect_training_data(total_actions):
    ...
    network_params = np.empty((total_actions, 7))
    ...
    for action_i in range(total_actions):
        ...
        #params = np.concatenate(sensor_readings, np.array([collision]).astype(int))
        network_params[action_i,:5]= sensor_readings.copy()
    ...
    np.savetxt("data.csv", network_params, delimiter=",")
